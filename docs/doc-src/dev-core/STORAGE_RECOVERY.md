...
所有 DataStore 必须通过 `recoverablePreferencesDataStore` 声明。注册器保证同一进程内每个文件名只有一个 actor，并为每个文件配置 `ReplaceFileCorruptionHandler`。
...
原始快照恢复在覆盖 live 目录前校验 manifest 的完整 payload 清单、重复 ZIP entry、payload 目录类型、Room 数据库和每个 ObjectBox `data.mdb`。ObjectBox 验证生成的 `lock.mdb` 会在复制前删除；缺少 `data.mdb` 的 ObjectBox 目录、被普通文件占位的 ObjectBox 目录、被目录占位的 Room 主库或 sidecar，以及缺少主数据库的 Room sidecar 都会拒绝导入。ZIP、全部数据库和替换前置条件完成验证之前，不关闭主进程已有的 DataStore、Room 或 ObjectBox owner；验证失败时当前设置页仍可继续使用。替换完成后必须同时失效 Room 与 ObjectBox 的进程内 preflight 状态，即使本进程从未构建 Room singleton，导入数据库也必须重新通过完整性和 schema 校验。
...
该脚本拒绝直接 `preferencesDataStore`、注册器外 `PreferenceDataStoreFactory.create`、未登记文件名、缺失 owner 和重复 owner。脚本自身测试位于 `ci/test/test_storage_ownership.py`。

Android 故障夹具与幂等修复断言位于 `app/src/androidTest/java/com/ai/assistance/operit/data/persistence/`。其中覆盖全部 cataloged Preferences、Room、ObjectBox、TTS、模型、角色、记忆空间和跨进程 Provider 写锁。按照仓库执行准则，只有在用户明确授权后才运行 Gradle 编译或测试。
