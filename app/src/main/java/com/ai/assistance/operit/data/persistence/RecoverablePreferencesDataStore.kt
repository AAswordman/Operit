...
fun recoverablePreferencesDataStore(
    ...
    require(name in PreferenceStoreCatalog.recoverable) {
        "Preferences DataStore is not recoverable: $name"
    }
    return managedPreferencesDataStore(name)
}

fun managedPreferencesDataStore(
    name: String
): ReadOnlyProperty<Context, DataStore<Preferences>> {
    require(name in PreferenceStoreCatalog.managed) { "Unregistered Preferences DataStore: $name" }
...
object RecoverablePreferenceDataStores {
    ...
    suspend fun preflightKnownStores(context: Context): List<String> {
        ...
        PreferenceStoreCatalog.recoverable.forEach { name ->
            try {
    ...
    suspend fun checkpointKnownStores(context: Context) {
        PreferenceStoreCatalog.recoverable.forEach { name ->
            try {
                val store = getHolder(context, name).store
                check(store is SnapshottingDataStore)
                store.checkpointCurrentState()
            } catch (e: Exception) {
    ...
    suspend fun checkpoint(context: Context, name: String, preferences: Preferences) {
        require(name in PreferenceStoreCatalog.recoverable) {
            "Preferences DataStore does not publish recovery snapshots: $name"
        }
    ...
    suspend fun quarantineLogicalState(
        ...
        require(name in PreferenceStoreCatalog.recoverable) {
            "Preferences DataStore does not support logical recovery: $name"
        }
        withContext(Dispatchers.IO) {
    ...
    private fun createStore(context: Context, name: String): Holder {
        ...
        val isRecoverable = name in PreferenceStoreCatalog.recoverable
        ...
        val store =
            if (isRecoverable) SnapshottingDataStore(context, name, delegate) else delegate
        return Holder(scope, store)
    }
