            ...
            if (hasLegacyState) {
                ...
                canonicalIds.forEach { id ->
                    ...
                    val decoded =
                        if (raw is String) {
                            try {
                                userPreferencesJson.decodeFromString<LegacyUserProfile>(raw)
                            } catch (e: Exception) {
                                AppLogger.e(
                                    "UserPreferencesManager",
                                    "Repairing invalid persisted legacy profile",
                                    e
                                )
                                null
                            }
            ...
            current.asMap().entries
                ...
                .forEach { (key, value) ->
                    ...
                    if (memorySpaceId == DEFAULT_PROFILE_ID ||
                        ...
                        if (value != true) {
                            removePreferenceByName(mutable, keyName)
                            mutable[booleanPreferencesKey(keyName)] = true
                            issues += keyName
                        }
                    }
                }
    ...
    suspend fun readLegacyUserProfiles(): LegacyUserProfileSnapshot {
        ...
        val profiles = ids.distinct().map { id ->
            ...
            if (encoded == null) {
                ...
                userPreferencesJson.decodeFromString<LegacyUserProfile>(encoded)
            }
        }
