...
object PreferenceStoreCatalog {
    ...
    init {
        check(managed.size == managed.distinct().size) { "Duplicate Preferences DataStore name" }
        check(recoverable.size == recoverable.distinct().size) {
            "Duplicate recoverable Preferences DataStore name"
        }
        check(recoverable.toSet() == managed.toSet() - TOKEN_STATS)
    }
}
